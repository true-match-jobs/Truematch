...
export const DashboardConversationsPage = () => {
  ...
  useEffect(() => {
    ...
    const loadConversations = async (showLoading = false) => {
      ...
      try {
        const peer = await chatService.getPeer();
        const history = await chatService.getMessages(peer.id);
        ...
        setConversations([
          {
            ...
            lastMessagePreview: formatLastMessagePreview(lastMessage.content),
      ...
      <div className="mt-4 min-h-0 flex-1 overflow-y-auto">
        ...
        <ul>
          {sortedConversations.map((conversation) => {
            const isLastMessageFromCurrentUser = conversation.lastMessageFromUserId === user?.id;
            const isPreviewUserOnline = Boolean(presenceByUserId[conversation.user.id]);
            const previewText = isLastMessageFromCurrentUser
                  ...
                  <div className="relative">
                    ...
                    <span
                      className={`absolute bottom-0 right-0 h-3 w-3 rounded-full border border-dark-bg ${
                        isPreviewUserOnline ? 'bg-emerald-400' : 'bg-zinc-500'
                      }`}
                      aria-hidden
                    />
                  </div>
